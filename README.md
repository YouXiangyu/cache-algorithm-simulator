...
**设计思路**
- 两个窗口尺寸交替：25 页与 45 页，每个阶段 2500 次请求。
- 在每个阶段内部继续滑动窗口，生成新的访问顺序。
- 不断交替阶段，直到累计 50000 次请求。

**关键特征**
- 小窗口期工作集小于缓存，大窗口期则显著大于缓存。
- 算法需要在“满缓存”和“超缓存”之间快速适配，检验对阶段性工作集的响应。

### WL05_FIFO_CONVOY — 污染+切换
...
**设计思路**
- 阶段 A：页面 1-10 共循环 10 次（100 次请求）。
- 阶段 B：32 页滑动窗口（32 次请求），窗口起点依据轮次偏移。
- 两个阶段组成 132 次请求的完整轮次，重复并裁剪到 50000。

**关键特征**
- 相邻阶段在“高频热集”和“纯滑窗”之间来回切换。
- ARC 可以通过幽灵队列反馈调整 T1/T2，兼顾频率与最近使用；其他算法需在两种信号之间不断妥协。

### WL07_SCAN_SANDWICH — 热浪 + 双滑窗 + 长扫描
...
**关键特征**
- 单轮内包含频率积累、短窗口、长窗口与冷扫描，模式切换比 WL06 更频繁。
...
| WL04 | 5.40 | 48.52 | 48.52 | 71.58 | 49.28 | 83.36 |
| WL05 | 3.14 | 99.87 | 99.87 | 99.81 | 99.81 | 99.87 |
| WL06 | 76.81 | 68.77 | 68.77 | 84.41 | 77.31 | 92.31 |
| WL07 | 33.42 | 24.18 | 24.18 | 32.81 | 33.42 | 33.68 |
| WL08 | 53.12 | 45.18 | 45.18 | 53.10 | 53.12 | 61.44 |
| WL09 | 44.53 | 69.87 | 69.87 | 70.55 | 71.63 | 71.69 |
